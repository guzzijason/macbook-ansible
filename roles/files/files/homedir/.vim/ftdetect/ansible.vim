...
autocmd BufNewFile,BufRead *.yml,*.yaml,*/{group,host}_vars/*,*/ansible_pull/*  call s:SelectAnsible("ansible")
...
fun! s:SelectAnsible(fileType)
  ...
  if fp =~ '/roles/.*\.y\(a\)\?ml$' || fp =~ '/\(group\|host\)_vars/' || fp =~ '/ansible_pull/.*\.y\(a\)\?ml$'
